...
<p>This functionality is under development and is not available in the current version of Thelia</p>
</div>
...
Some attributes here are optional. ```name="hook.event_listener"``` must be defined as well. The attribute ```event``` is the name of the hook to which it will be associated. The ```type``` attribute indicates the context of the hook : frontOffice (default), backOffice, pdf or email. At last, ```method``` attribute indicates the method that will be called. By default, it will be based on the hook name. eg : for ```product.additional``` hook, the method ```onProductAdditional``` will be called (*CamelCase prefixed by on*).  
Finally, ```active``` allows you to activate, or not, the hook once the module is installed.
...
When a hook is called from the template, an event for this hook is created and dispatched by Thelia. If your module listens to this hook, the method indicated in your ```config.xml``` is called, and its argument is the event generated.
...
We're going to display a message in the product page if the product is already in the customer cart.
...
This is not very useful right now but you can imagine whatever you want.
...
You can also notice that the arguments of Smarty hook function/hook are accessible from the event itself thanks to the ```getArgument``` method. The event also contains the code (```getCode```) of the current event. *It's useful when several hooks point to the same method.*


### Example of a hook block

We're going to add a new tab to the additional area of the product page. In this new tab we're going to display related content to the current product.
...
Et voilà ! 
The associated contents are now displaied in a new tab.

Here is the associated template in product page :
...
The ```BaseHook``` class provides the ```render``` method that does the work for you. Its behavior is not complicated.

First, you have to put your templates inside the `Template` directory. The location of the template also depends on the type (context) of the hook. If it's a ```frontOffice``` hook then the root path of your template will be ```local/modules/MyModule/templates/frontOffice/default/```
...
We've added a system of overriding for assets, that allows you to redefine the asset in the template you use. For example, if you use the default template, and use the image ```img/more.png``` in your smarty template, you can override this image in your template by putting your own image in ```template/frontOffice/default/modules/MyModule/img/more.png```


### Adding CSS and JS files.

For some module, you will have to load some specific dependencies like CSS stylesheet or JavaScript.

For **CSS stylesheets**, you have to add the ```<link>``` tag in the ```<head>``` of our page. In order to do this, you have to subscribe to specific ```hook``` located in this area. 
If the stylesheet is the same for all your websit, the best hook is the ```main.stylesheet``` hook. 
If the stylesheet is needed juste for a few pages, you can use a specific hook : **product**.stylesheet, **category**.stylesheet, ... 


Regarding JavaScript files, you should add them at the bottom of the page and after the inclusion of the default JavaScript files of your template. The hook for global JavaScript is : ```main.after-javascript-include``` and for specific page : **product**.after-javascript-include, **category**.after-javascript-include, ...

Once done, your method is able to add files to the hook event that is dispatched for these hooks :
